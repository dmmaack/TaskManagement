...
public class UsersController : ControllerBase
{
    ...
    [HttpPost]
    //[Authorize(Roles = "1")]
    [Route("CreateUserRabbitMQ")]
    public async Task<IActionResult> CreateUserRabbitMQ(ProducerUsersRabbitMQCommand userCommand)
    {
        var objResponse = await _mediator.Send(userCommand);

        if (!objResponse.Success)
            return CreatedAtAction(nameof(CreateUser), new { id = 0 }, objResponse);
